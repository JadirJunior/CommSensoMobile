...
class _AppShellPageState extends State<AppShellPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      int index = navigationController.currentIndex.value;
      ...
      return Scaffold(
        ...
        // preserva estado das abas
        bottomNavigationBar: NavigationBar(
          selectedIndex: navigationController.currentIndex.value,
          onDestinationSelected: (i) =>
              setState(() => navigationController.goToPage(i)),
          destinations: [
            NavigationDestination(
                icon: const Icon(Icons.devices_other), label: _titles[0]),
            NavigationDestination(
                icon: Icon(Icons.timeline), label: _titles[1]),
            // NavigationDestination(icon: Icon(Icons.apps), label: _titles[2]),
            NavigationDestination(
                icon: Icon(Icons.settings), label: _titles[2]),
          ],
        ),
